# This script produces a long tidy dataset based on the data from the Human
...
# Creates a long tidy data set with the average of each variable for each
#activity and each subject (Assignment step 5)
dataset_melt <- melt(dataset_mean_std, id = c("Subject", "Activity"),
                     measure.vars = (names(dataset_mean_std)[3:81]),
                     variable.name = "Measure")
longtidydata <- dataset_melt %>% group_by(Subject, Activity, Measure) %>% 
    summarise_each(funs(mean))
longtidydata <- arrange(longtidydata, as.numeric(as.character(Subject)),
                        Activity)
names(longtidydata)[4] <- "Average"

# Creates a .txt file with the long tidy data table
write.table(longtidydata, "longTidyData.txt", row.names = FALSE)
